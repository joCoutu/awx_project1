...
- name: Entrypoint
  hosts: all
  roles:
    - { role: $NEW_ROLE } " > roles/$NEW_ROLE/tests/test.yml

echo "galaxy_info:
  author: Jonathan Coutu
  ...
  platforms:
    - name: Debian
      release: bullseye
  galaxy_tags: [$NEW_ROLE, debian, jocoutu]
dependencies: []" > roles/$NEW_ROLE/meta/main.yml

b 'localhost,' $NEW_ROLE  # test $NEW_ROLE (do noting)
```
